...
export async function getUserMeetings(type = "upcoming") {
    ...
    // From today, I want to show upcoming and past meetings
    const now = new Date()

    const meetings = await db.booking.findMany({
        where: {
            userId: user.id,
            // If it is upcoming, we should take the time greater than now. Else it will be less than now
      ...
      event: {
        ...
        orderBy: {
        // If it is upcoming events, order by ascending else descending
